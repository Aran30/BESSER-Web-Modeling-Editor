...
BESSER WME supports realtime collaboration by tracking and emitting patches when changes happen to the diagram. The patches are in the format of `RFC 6902`_ standard (i.e. `JSON Patch`_), so that they can be applied to BESSER WME diagrams by clients in different languages and in various stacks (i.e. other clients, on the server, etc.).
